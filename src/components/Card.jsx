...
export default function Card({ item, index }) {
    ...
    if (recipe.groupBy !== null || recipe.chartType === 'bar' || recipe.chartType === 'line' || recipe.chartType === 'area') {
        allowedTypes.push('bar');
        if (recipe.groupBy && (recipe.groupBy.includes('Date') || recipe.groupBy.includes('month') || recipe.groupBy.includes('quarter'))) {
            allowedTypes.push('line', 'area');
        } else {
            allowedTypes.push('line');
        }
    }

    if (recipe.secondMetric === null && recipe.groupBy !== null) {
        allowedTypes.push('pie', 'donut');
    }
    ...
    // ensure active is in allowed, if not add it
    ...
    const iconMap = {
        ...
        'scatter': <BarChart2 size={12} /> // fallback icon
    };
    ...
    return (
        <div className="dashboard-card" style={{ animationDelay: `${index * 0.08}s` }}>
            ...
            <div className="card-header">
                ...
                <p className="card-query">"{userQuery}"</p>
            </div>
            ...
            <div className="card-meta">
                ...
                {recipe.groupBy && <div className="meta-pill">by {recipe.groupBy}</div>}
