...
const COLORS = ['#e8175d', '#ffd166', '#06d6a0', '#4cc9f0', '#a78bfa', '#ff9f43', '#ff6b9d'];
...
const CustomTooltip = ({ active, payload, label, recipe }) => {
    if (active && payload && payload.length) {
        return (
            <div style={{
                background: '#1a1a28', border: '1px solid #1f1f30',
                borderRadius: '10px', padding: '10px 14px',
                fontFamily: 'Outfit', fontSize: '13px', color: '#f0f0f8'
            }}>
...
export default function Chart({ recipe, data }) {
    ...
    const xAxis = <XAxis dataKey="name" tick={{ fill: "#52526a", fontSize: 11 }} angle={-30} textAnchor="end" height={50} />;
    const yAxis = <YAxis tick={{ fill: "#52526a", fontSize: 11 }} width={72} tickFormatter={(v) => formatValue(v, metric)} />;
    const grid = <CartesianGrid stroke="#1f1f30" strokeDasharray="4 4" vertical={false} />;
    const tooltip = <Tooltip content={<CustomTooltip recipe={recipe} />} cursor={{ fill: 'rgba(232,23,93,0.05)' }} />;

    if (chartType === 'table') {
        return (
            <div style={{ height: 290, overflowY: 'auto', paddingRight: 10 }}>
